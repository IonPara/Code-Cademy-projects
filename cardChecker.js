 ...
 for(let i = 1 ; i < arr.length; i+=2){
   ...
   if((arr[i] * 2) > 9){
     arr[i]= (arr[i] * 2) - 9;
      ...
      arr.reverse();
    let arrReduce = arr.reduce(reducer);
let result =  arrReduce % 10 === 0;
return result;  
};
...
for(i=0; i < invalidCardCompanies.length; i++){
if(uniqueArray.indexOf(invalidCardCompanies[i]) === -1) {
    uniqueArray.push(invalidCardCompanies[i]);
   };
    };
